...
-- Create select policy with proper validation (without turno restriction)
...
-- Create update policy (without turno restriction)
...
-- Update validation function (without turno validation)
...
DECLARE
    v_voucher RECORD;
BEGIN
    -- Find and validate voucher (without turno check)
    SELECT *
    INTO v_voucher
...
'Permite visualizar apenas vouchers válidos, não utilizados e dentro do horário permitido da refeição';
...
'Valida e registra o uso de vouchers descartáveis respeitando apenas o horário da refeição';