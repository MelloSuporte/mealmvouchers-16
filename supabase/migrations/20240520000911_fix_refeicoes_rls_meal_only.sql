...
-- Create SELECT policy (permite visualização apenas de refeições, não tipos)
...
COMMENT ON POLICY "refeicoes_insert_policy" ON refeicoes IS 'Permite que apenas admins com permissão específica insiram refeições';
COMMENT ON POLICY "refeicoes_update_policy" ON refeicoes IS 'Permite que apenas admins com permissão específica atualizem refeições';